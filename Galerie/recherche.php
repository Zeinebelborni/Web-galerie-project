...
session_start();
 ...
 {   $sql = "SELECT * FROM tableaux WHERE categorie = '".$_POST['categorie'] ."' and size='".$_POST['size']."'";
  $result = mysqli_query($db,$sql);
    ...
    {    
      ...
      {   
        $_SESSION['art']=$tableau['artiste'];
        $_SESSION ['cat']=$tableau['categorie'];
        $_SESSION ['p']=$tableau['prix'];
        $_SESSION['s']=$tableau['size'];
        $_SESSION['li']=$tableau['lien'];
        /*$uname=$_SESSION['User'];*/ 
        echo" <p><a target='_blank' href='#' > <img src=".$tableau['lien']."   USEMAP='#Map' onclick='image(this)'></a> <br />Artist:". $tableau['artiste'] ."/ Price:" .$tableau['prix']." <input name='add_btn' type='submit' value='Add to basket'  class='buttstyle' /><p>";
          ...
          <br></br>
                      <p class="txt"><?php
                                    ...
                                    if($_SESSION['added']==true)
                                       {
                                        echo ' The painting is added to your basket successfully <br/>';
